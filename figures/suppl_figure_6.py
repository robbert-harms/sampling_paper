...
def create_plots(model_name, thinning_samples, thinning_methods_mean, thinning_methods_std, display_legend=True):
    ...
    if display_legend:
        ax_chain.legend(loc="lower right", bbox_to_anchor=(1.02, 1.02), borderaxespad=0.,
                        ncol=3, handletextpad=0.6, frameon=False)
        ax_mean.legend(loc="lower right", bbox_to_anchor=(1.02, 1.02), borderaxespad=0.,
                       ncol=3, handletextpad=0.6, frameon=False)
...
def load_samples(model_name):
    if model_name == 'NODDI':
        return mdt.load_sample(pjoin('suppl_figure_6', model_name, 'samples', 'w_ic.w'))
    else:
        samples = mdt.load_samples(pjoin('suppl_figure_6', model_name, 'samples'))
