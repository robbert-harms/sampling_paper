            ...
            for model_name in model_names:
                for snr in noise_snrs:
                    ...
                    for trial_ind in range(nmr_trials):
                        current_pjoin = pjoin.create_extended(protocol_name, model_name, 'output', str(snr),
                                                              'sampling_methods',
...
def plot_protocol_results(data, model_name):
    ...
    f.savefig(mdt.make_path_joiner('/tmp/sampling_paper/adaptive_proposals/ess/all/', make_dirs=True)('{}.png'.format(model_name)))
