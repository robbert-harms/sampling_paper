...
def create_plots(model_name, method_samples, method_means, method_stds, display_legend=False, plot_mean_std=True):
    ...
    if display_legend:
        ax_chain.legend(loc="lower right", bbox_to_anchor=(1.05, 1.02), borderaxespad=0.,
                        ncol=3, handletextpad=0.5, frameon=False)
...
for model_name in ['BallStick_r1', 'NODDI']:
    ...
    for method in methods:
        samples = mdt.load_sample(pjoin('suppl_figure_5', 'init_points', method, model_name, 'samples', param_names[model_name]))
